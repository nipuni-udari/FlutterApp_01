...
$qrt = "SELECT * FROM inquiry_details WHERE inquiry_last_status='CONFIRMED' and logged_by = '" . mysqli_real_escape_string($con_hel, $_POST['userHris']) . "' ORDER BY inquiry_dateTime desc";
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $qrt2 = "SELECT SUM(total_value) AS total_value 
                 ...
                 WHERE inquiry_last_status ='CONFIRMED' and logged_by = '" . mysqli_real_escape_string($con_hel, $_POST['userHris']) . "' and inquiry_refno = '" . mysqli_real_escape_string($con_hel, $row['inquiry_refno']) . "'";
